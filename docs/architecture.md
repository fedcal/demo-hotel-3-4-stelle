...
- **`outputMode: "server"`** — produce sia `dist/hotel-3-4-stelle/browser/` (statico) sia `dist/hotel-3-4-stelle/server/server.mjs` (Node)
...
`src/assets/mock/*.json` — file statici importati via `HttpClient` o `fetch` lato client, oppure via direct `import` lato server (SSR). Vedi pagina [Mock Data](/mock-data) per la struttura specifica di Hotel 3-4 stelle.
